...
DrawTool.polygon = new L.polygon(DrawTool.points,{fillColor: 'red'});
DrawTool.tempPolygon = new L.polygon([],{color: 'none',fillColor: 'blue'});//
...
DrawTool.init = function (map,iconUrl){
	...
	function onClick(e) {
		var marker;
		if(iconUrl){
			marker = L.marker(e.latlng,{icon: L.icon({iconUrl: iconUrl, iconSize: [25,25]})});
		}else{
	...
	function onMove(e) {
		if (DrawTool.points.length > 0) {
			...
			DrawTool.tempPolygon.setLatLngs(DrawTool.points);
			DrawTool.tempPolygon.addTo(map);
		}
	}
	...
	function clickMarker(e){
		...
		DrawTool.polygon = L.polygon(DrawTool.points,{fillColor: 'red'});
		DrawTool.polygon.addTo(map);
		DrawTool.polygon.on('dblclick', DrawTool.onDoubleClick);
		// 移动标记（微调）
	...
	function resetRegion(){
		...
		DrawTool.lines .remove();
		DrawTool.polygon = L.polygon(DrawTool.points);
		DrawTool.polygon.on('dblclick', DrawTool.onDoubleClick);
		DrawTool.polygon.addTo(map);
	}
	...
	function cancle(e){
		...
		DrawTool.tempPolygon.remove();
		DrawTool.polygon.remove();
		DrawTool.polygon = new L.polygon(DrawTool.points);
		DrawTool.lines  = new L.polyline(DrawTool.points);
		DrawTool.tempLines = new L.polyline([],{dashArray: 12});
		DrawTool.tempPolygon = new L.polygon([],{color: 'none',fillColor: 'blue'});
	}
